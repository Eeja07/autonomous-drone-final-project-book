...
%
%Hasil pengujian menunjukkan bahwa integrasi antara Raspberry Pi 5, Pixhawk, MAVSDK, GStreamer, ONNX Runtime, dan frontend monitoring berhasil berjalan secara sinkron.
%State machine mampu menerjemahkan hasil deteksi YOLO menjadi perintah VelocityNedYaw secara real-time selama mode OFFBOARD aktif.
%Meskipun demikian, beberapa keterbatasan masih ditemukan, antara lain:
%
%\begin{itemize}
%	\item inferensi masih berbasis CPU,
%	\item belum terdapat AI accelerator eksternal,
%	\item belum menggunakan gimbal stabilizer,
%	\item dan kualitas deteksi masih dipengaruhi pencahayaan lingkungan.
%\end{itemize}
%
%Secara keseluruhan, sistem berhasil memenuhi kebutuhan minimum operasi drone otonom berbasis edge computing untuk misi pencarian korban secara real-time.

\section{Evaluasi Keberhasilan Sistem}

%Berdasarkan seluruh hasil pengujian, sistem drone otonom berhasil memenuhi kebutuhan minimum operasi \textit{real-time} berbasis \textit{edge computing}.
%Integrasi antara sistem deteksi manusia, komunikasi MAVSDK, video streaming, dan frontend monitoring berhasil berjalan secara sinkron pada Raspberry Pi 5.
%
%Pendekatan optimasi menggunakan ONNX Runtime, mode \textit{headless}, serta resolusi inferensi 320×320 terbukti mampu meningkatkan performa sistem secara signifikan dibandingkan konfigurasi awal.
%Selain itu, implementasi regulator UBEC 5A berhasil menjaga kestabilan suplai daya selama inferensi AI dan operasi drone berlangsung secara simultan.
%Arsitektur \textit{multithreading} juga mampu mempertahankan kontinuitas komunikasi telemetry dan video streaming tanpa mengganggu proses inferensi utama.
%Meskipun demikian, sistem masih memiliki beberapa keterbatasan, terutama pada kondisi pencahayaan ekstrem, objek manusia berukuran sangat kecil, dan inferensi yang masih sepenuhnya bergantung pada CPU Raspberry Pi 5. Oleh karena itu, pengembangan lanjutan masih diperlukan terutama pada sisi akselerasi inferensi dan stabilisasi visual.