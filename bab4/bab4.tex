...
Tabel \ref{tab:hardware_testing} merangkum spesifikasi perangkat keras yang digunakan dalam proses pengujian sistem. Berdasarkan konfigurasi tersebut, interkoneksi fisik antar-perangkat dirancang untuk menahan getaran dan beban terbang drone secara dinamis. Dapat diamati bahwa pemilihan komponen ini memenuhi standar keandalan sistem tertanam (\textit{embedded system}) yang dibutuhkan di lapangan. Dengan demikian, ketersediaan perangkat keras yang andal ini menjadi fondasi utama dalam melaksanakan pengujian perangkat lunak sistem.
...
Data perbandingan iterasi daya pada Tabel \ref{tab:iterasi_daya} menunjukkan evolusi desain catu daya dari model awal hingga model final yang stabil. Berdasarkan pengujian tersebut, perubahan konfigurasi dilakukan untuk mengatasi kendala listrik yang dapat menghentikan operasi Raspberry Pi 5. Dapat diamati bahwa penggunaan UBEC 5A memberikan stabilitas tegangan terbaik saat sistem mengalami lonjakan beban komputasi. Dengan demikian, konfigurasi daya ini dipilih sebagai standar catu daya utama untuk mendukung penerbangan otonom drone.
...
Pelatihan model dilakukan menggunakan YOLOv8n pretrained dengan dataset COCO2017 kelas \textit{person-only}. Pemanfaatan bobot awal (pretrained) ini bertujuan untuk mempercepat proses konvergensi model pada kelas spesifik manusia. Adapun seluruh pengaturan hiperparameter dan lingkungan komputasi yang diterapkan selama proses ini ditunjukkan pada Tabel \ref{tab:training_config}.
...
Pengujian ini dilakukan untuk memastikan seluruh arsitektur perangkat lunak mampu berjalan secara bersamaan tanpa saling memblokir (blocking) pada Raspberry Pi 5 selama operasi drone berlangsung. Fokus utama diarahkan pada sinkronisasi pembagian memori antar-thread untuk menghindari race condition. Dengan demikian, keandalan fungsional dari seluruh sistem asinkron backend-frontend dapat dipastikan aman sebelum uji terbang otonom.
...
Gambar \ref{fig:loggstreamer} dan Gambar \ref{fig:logyolo} menunjukkan bahwa integrasi pemrosesan video dan inferensi YOLOv8n dapat berjalan secara paralel tanpa mengalami kemacetan antrean data. Berdasarkan hasil log tersebut, thread konsumen visual mampu memproses setiap frame gambar dengan waktu inferensi yang stabil. Dapat diamati bahwa waktu komputasi inferensi ini tetap berada dalam batas toleransi real-time yang aman. Dengan demikian, sinkronisasi aliran data video ini siap dihubungkan dengan modul pengiriman telemetri sistem.
...
Gambar \ref{fig:logtelemmavsdk} dan Gambar \ref{fig:logsocket} memvalidasi kelancaran transmisi telemetri nirkabel dari flight controller hingga ke dashboard pemantauan operator. Berdasarkan hasil log SocketIO tersebut, pembaruan status parameter terbang dikirim secara berkala tanpa adanya kehilangan paket data. Dapat diamati bahwa latensi pengiriman data telemetri ini sangat minimal sehingga mendukung pemantauan real-time yang responsif. Dengan demikian, sistem komunikasi backend ini siap diintegrasikan dengan komponen penampil antarmuka pengguna.
...
Gambar \ref{fig:uiweb1} dan Gambar \ref{fig:uiweb2} membuktikan bahwa dasbor React 19 mampu merender aliran video dan status penerbangan drone secara dinamis. Berdasarkan implementasi visual tersebut, operator dapat memantau posisi spasial dan status state machine drone secara real-time. Dapat diamati bahwa responsivitas antarmuka tetap terjaga meskipun drone sedang melakukan manuver otonom yang kompleks. Dengan demikian, keandalan fungsional dasbor ini siap diuji di bawah pengaruh variasi kualitas sinyal jaringan nirkabel.
...
Hasil pengujian dan evaluasi fungsional dari seluruh subsistem perangkat lunak serta perangkat keras di atas wahana tersebut menjadi landasan penting dalam menguji keandalan sistem secara terpadu. Selanjutnya, integrasi sistem drone otonom ini divalidasi secara menyeluruh melalui serangkaian skenario misi penerbangan nyata di lapangan.
...
Pemetaan antara skenario pengujian yang dirancang dengan sesi penerbangan yang mengeksekusinya ditunjukkan pada Tabel \ref{tab:pemetaan_skenario}. Setiap skenario uji ditandai dengan kode huruf alfabet dari A hingga I untuk mempermudah kategorisasi fokus pengamatan. Pembagian sesi penerbangan nyata ini disesuaikan dengan kondisi baterai dan alokasi prioritas pengujian lapangan.
...
Pengujian validasi FSM sebelumnya dilakukan menggunakan target dengan posisi berdiri. Sebagai pengujian tambahan, pengujian juga dilakukan dengan menggunakan variasi target dalam posisi duduk dan tidur. Pengujian ini bertujuan untuk memvalidasi bahwa transisi state machine dari SCAN $\rightarrow$ DETECT $\rightarrow$ APPROACH $\rightarrow$ DOCUMENT $\rightarrow$ DISPLACING $\rightarrow$ RTL tetap berjalan dengan valid tanpa terpengaruh oleh variasi posisi tubuh target.
...
Gambar~\ref{fig:fsm_duduk} dan Gambar~\ref{fig:fsm_tidur} mendemonstrasikan stabilitas deteksi model YOLOv8n hasil \textit{fine-tuning} terhadap variasi orientasi target manusia yang meliputi posisi berdiri, duduk, berjalan, dan tidur. Berdasarkan hasil visualisasi tersebut, kestabilan kotak pembatas (\textit{bounding box}) bervariasi bergantung pada kelengkapan fitur visual tubuh korban yang tertangkap oleh sensor kamera. Pada orientasi tegak seperti berdiri dan berjalan, deteksi menunjukkan konsistensi tinggi karena pola geometris tubuh manusia sangat selaras dengan dataset pelatihan. Sebaliknya, pada posisi duduk dan terutama tidur, stabilitas deteksi dipengaruhi oleh sudut datang (\textit{angle of attack}) kamera drone yang berpotensi menimbulkan oklusi parsial pada bagian ekstremitas tubuh korban. Dengan demikian, meskipun terdapat variasi orientasi target, model hasil optimasi ini tetap mampu mempertahankan kestabilan deteksi yang memadai untuk mentransisikan keadaan otonom secara tepat.
...
Pada posisi tidur ditemukan karakteristik yang berbeda. Deteksi manusia cenderung berhasil apabila kamera dapat menangkap sebagian besar bentuk tubuh manusia, mulai dari kepala hingga kaki, terutama ketika drone mengamati target dari sisi kanan atau sisi kiri tubuh. Sebaliknya, ketika drone berada pada orientasi yang hampir sejajar dengan sumbu tubuh target, misalnya tepat di belakang kepala atau tepat di depan kaki, objek manusia sering kali tidak terdeteksi karena bentuk tubuh yang terlihat pada citra menjadi sangat terbatas dan tidak menyerupai representasi manusia yang umum dipelajari model. Seiring pergerakan drone selama proses pencarian, sudut pandang kamera berubah sehingga bagian kepala, badan, dan kaki mulai terlihat secara bersamaan dari sisi tubuh target. Pada kondisi tersebut deteksi kembali berhasil dilakukan dan FSM dapat melanjutkan transisi ke tahapan misi berikutnya. Hasil ini menunjukkan bahwa keberhasilan sistem tidak hanya dipengaruhi oleh posisi tubuh target, tetapi juga oleh sudut pengamatan kamera terhadap target selama proses pencarian berlangsung.
...
Untuk memberikan gambaran distribusi latency yang lebih representatif, dilakukan pula analisis persentil terhadap latency end-to-end sebagaimana ditunjukkan pada Tabel \ref{tab:e2e_percentile}. Pembagian persentil ini membantu memetakan batas atas waktu tunggu (latency) pada sebagian besar sampel data pengujian. Melalui analisis statistik sebaran data ini, stabilitas pemrosesan asinkron drone otonom dapat dibuktikan secara lebih valid.
...
Data persentil pada Tabel \ref{tab:e2e_percentile} menunjukkan sebaran latensi pemrosesan sistem dari hulu ke hilir secara komprehensif. Berdasarkan analisis distribusi tersebut, mayoritas siklus pemrosesan terkonsentrasi pada rentang waktu yang aman di bawah batasan kritis 200 ms. Dapat diamati bahwa nilai pencilan ekstrem hanya terjadi pada persentil tertinggi akibat beban komputasi transien dari sistem operasi. Dengan demikian, karakteristik latensi yang stabil ini memberikan jaminan responsivitas kendali navigasi otonom yang presisi. Kestabilan waktu tanggap ini menjadi parameter penentu keberhasilan manuver kendali drone. Selanjutnya, stabilitas respons navigasi tersebut dievaluasi secara geometris melalui skenario penentuan akurasi pendekatan dan perpindahan spasial drone otonom.
...
Gambar \ref{fig:battery_rtl}, Gambar \ref{fig:pre_rtl_exit}, dan Gambar \ref{fig:rc_override} menunjukkan log keberhasilan aktivasi sistem keselamatan saat terjadi kondisi darurat maupun intervensi manual. Berdasarkan log tersebut, drone secara andal menghentikan misi otonom dan mengembalikan kendali penuh kepada pilot keselamatan. Dapat diamati bahwa transisi mode terbang ini berjalan secara instan tanpa adanya penundaan kontrol yang membahayakan. Dengan demikian, keandalan sistem pengaman ini memberikan jaminan keselamatan tingkat tinggi selama pengujian misi pencarian korban secara berkelanjutan.
...
Gambar \ref{fig:persistent_scout} memperlihatkan dokumentasi korban yang berhasil diidentifikasi selama pengujian misi pencarian otonom yang berkelanjutan. Berdasarkan hasil dokumentasi visual tersebut, sistem terbukti mampu mengenali target manusia pada berbagai pose dan kondisi pencahayaan alami di lapangan. Dapat diamati bahwa kualitas gambar hasil dokumentasi cukup jelas untuk keperluan verifikasi identitas korban oleh tim penyelamat. Dengan demikian, keberhasilan perekaman visual ini melandasi pengujian kestabilan pelacakan target yang dinamis. Pelacakan dinamis ini membutuhkan algoritma smoothing agar koordinat target tidak memicu getaran pada motor servo gimbal atau respon navigasi. Oleh karena itu, kestabilan penguncian target dan peredaman fluktuasi dievaluasi secara khusus pada bagian berikutnya.
...
Secara keseluruhan, hasil pengujian menunjukkan bahwa mekanisme \textit{target locking} mampu mempertahankan target secara stabil selama operasi nyata, termasuk ketika terdapat lebih dari satu kandidat target pada bidang pandang kamera. Tidak ditemukan kejadian \textit{unnecessary lock switching} selama pengujian, sehingga sistem mampu menjaga konsistensi pemilihan target sepanjang siklus navigasi otonom. Konsistensi pemrosesan target yang stabil ini membutuhkan kinerja komputasi yang intensif pada unit onboard. Untuk memastikan kontinuitas operasi tersebut, konsumsi memori dan suhu operasional perangkat keras dievaluasi secara berkala pada skenario pengujian berikutnya.
...
Hasil pengujian menunjukkan bahwa Raspberry Pi 5 mampu menjalankan seluruh komponen sistem otonom secara bersamaan tanpa mengalami kegagalan proses maupun kejadian \textit{throttling} yang terdeteksi. Meskipun beban CPU relatif tinggi akibat inferensi visi komputer yang berjalan terus-menerus selama misi, penggunaan RAM tetap rendah dibandingkan kapasitas total sistem, sehingga masih tersedia ruang sumber daya yang cukup untuk pengembangan fitur tambahan pada implementasi berikutnya. Kestabilan performa termal ini menjamin keberlanjutan operasi komputasi di bawah berbagai kondisi lingkungan. Skenario pengujian terakhir mengevaluasi ketahanan deteksi visual di bawah pengaruh fluktuasi intensitas cahaya matahari dan kegelapan malam.
...
Berdasarkan data pada Tabel~\ref{tab:env_light}, tingkat keberhasilan deteksi YOLOv8n hasil \textit{fine-tuning} menunjukkan variasi performa yang signifikan pada kondisi pencahayaan pagi, siang, dan sore hari. Performa deteksi mencapai titik paling optimal pada siang hari karena intensitas cahaya yang merata meminimalisir bayangan dan derau (\textit{noise}) visual pada citra. Namun, tantangan utama muncul pada kondisi malam hari di mana sistem dihadapkan pada limitasi pencahayaan yang ekstrem. Pada kondisi malam gelap tanpa bantuan cahaya buatan, model sama sekali tidak mampu mengidentifikasi target akibat hilangnya fitur visual manusia secara total. Sebaliknya, pada kondisi malam dengan iluminasi lampu sorot atau lampu penerangan jalan, tingkat deteksi mengalami peningkatan kembali meskipun masih berada di bawah performa siang hari. Dengan demikian, ketersediaan intensitas cahaya yang cukup terbukti menjadi parameter krusial dalam menjamin stabilitas dan akurasi sistem visi komputer pada drone.
