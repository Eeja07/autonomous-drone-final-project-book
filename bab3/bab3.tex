...
\begin{enumerate}
	...
	\item Framework Web: Flask (Backend), React 19 (Frontend), dan SocketIO untuk komunikasi data \textit{real-time}.
...
Siklus pencarian korban merupakan perilaku utama drone selama mode otonom aktif. Pada tahap ini drone melakukan proses pencarian manusia secara otonom menggunakan data visual dari kamera. Perilaku sistem dirancang menggunakan pendekatan state machine sehingga drone mampu melakukan transisi tindakan secara dinamis berdasarkan kondisi lingkungan dan hasil deteksi visual secara \textit{real-time}.
...
Pada tahap APPROACH, drone bergerak mendekati target berdasarkan posisi objek terhadap pusat frame kamera. Pergerakan dilakukan secara bertahap agar kestabilan penerbangan tetap terjaga selama proses pendekatan berlangsung. Setelah target berada pada jarak tertentu dan memenuhi kondisi dokumentasi, sistem memasuki tahap DOCUMENT untuk melakukan pengambilan data visual terhadap manusia yang terdeteksi.
...
Setelah target berhasil diproses, hasil deteksi didistribusikan menuju sistem navigasi otonom dan sistem monitoring. Informasi deteksi digunakan oleh sistem otonom untuk menentukan perilaku navigasi drone, sedangkan pada sistem monitoring, data digunakan untuk menampilkan hasil deteksi kepada operator secara \textit{real-time}. Distribusi informasi ini memungkinkan sistem kontrol dan sistem monitoring berjalan secara bersamaan tanpa saling mengganggu proses utama drone selama operasi berlangsung.

\subsection{Perancangan Mesin Pengambil Keputusan}
Mesin pengambil keputusan merupakan bagian utama sistem otonom yang bertugas menentukan tindakan drone berdasarkan kondisi sistem dan hasil deteksi manusia. Pada tahap ini sistem mengintegrasikan data telemetri penerbangan dan informasi deteksi visual untuk menentukan aksi navigasi yang sesuai selama misi berlangsung. Pendekatan ini memungkinkan drone merespons perubahan kondisi lingkungan secara otonom dan \textit{real-time} sehingga perilaku sistem dapat tetap stabil selama operasi pencarian berlangsung.
...
Gambar \ref{fig:blokdiagram5} menunjukkan arsitektur mesin pengambil keputusan otonom yang bekerja berdasarkan evaluasi kondisi sistem dan kejadian dinamis. Logika pengambil keputusan ini membagi proses menjadi tiga tahap utama secara sekuensial: monitoring kondisi sistem untuk mengumpulkan data telemetri, evaluasi kejadian (\textit{event}) untuk menentukan prioritas kondisi, dan pemilihan tindakan otonom yang sesuai. Struktur ini dirancang untuk mempertahankan keamanan penerbangan dengan memprioritaskan keselamatan drone apabila terdeteksi kondisi kritis di lapangan.
...
Setelah proses evaluasi selesai dilakukan, sistem menentukan tindakan navigasi yang akan dijalankan drone. Tindakan yang dapat dipilih meliputi rotasi pencarian, pendekatan target, dokumentasi manusia, pergeseran posisi otonom, maupun prosedur keselamatan (\textit{safety procedure}) seperti Return to Launch (RTL). Pemilihan tindakan dilakukan secara otonom berdasarkan kondisi sistem dan state aktif pada state machine sehingga drone mampu menyesuaikan perilaku secara dinamis selama operasi berlangsung.
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			Menggunakan dua baterai LiPo terpisah. Satu baterai didedikasikan untuk PDB dan sistem propulsi, sementara satu baterai sekunder dihubungkan khusus untuk menyuplai UBEC 5A ke Raspberry Pi 5. Konfigurasi ini bertujuan untuk mengisolasi daya secara penuh guna meminimalkan pengaruh fluktuasi tegangan saat motor menarik arus maksimal, meskipun menambah beban angkut keseluruhan.
		\end{enumerate}
	\end{enumerate}
	...
	\end{enumerate}
	Seluruh pengambilan citra tetap menggunakan antarmuka USB 3.0 khusus untuk \textit{webcam} Logitech C922 guna menyediakan \textit{bandwidth} memadai dan meminimalkan latensi yang dibutuhkan untuk pemrosesan algoritma YOLOv8 secara \textit{real-time}.
...
Setelah seluruh komponen modular disempurnakan pada tahap refinement, integrasi akhir dirancang dalam bentuk assembly CAD final yang siap diimplementasikan secara fisik pada drone.
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\caption{Hasil Realisasi Casing Baterai}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.45\textwidth}
		\centering
		\includegraphics[width=0.9\linewidth]{bab3/tutup_baterai_real}
		\caption{Hasil Realisasi Tutup Casing Baterai}
	\end{subfigure}
	\caption{Hasil Realisasi Casing dan Tutup Casing Baterai (Desain Final)}
...
\begin{itemize}
	...
	\item MAVSDK Thread: Berjalan sebagai \textit{daemon thread} yang didedikasikan untuk \textit{asyncio event loop}. \textit{Thread} ini mengelola komunikasi telemetri dari \textit{flight controller} dan menjalankan algoritma otonom pada frekuensi tinggi (20Hz) tanpa interupsi.
...
\begin{figure}[H]
	...
	\label{fig:flowyolo}
\end{figure}
Mekanisme deteksi objek dan penguncian target (\textit{target lock}) secara berurutan ditunjukkan pada diagram alir Gambar \ref{fig:flowyolo}.
...
\begin{itemize}
	...
	Nilai ambang batas sebesar 15\% ditetapkan sebagai parameter keselamatan operasional sistem. Nilai tersebut dipilih secara empiris berdasarkan konfigurasi pengujian yang digunakan pada penelitian ini. Threshold digunakan untuk membantu memastikan tersedianya energi yang memadai selama:
	\begin{itemize}
...
\end{figure}
Struktur integrasi fisik dan logis antar komponen pada drone secara menyeluruh dipetakan melalui blok diagram pada Gambar \ref{fig:blokfullarsitektur}.
...
\begin{enumerate}
	...
	Gambar \ref{fig:blokvisualtocontrol} menunjukkan alur transmisi data dari kamera hingga perintah kontrol yaw dan kecepatan otonom dihasilkan. Berdasarkan implementasi tersebut, terdapat pemrosesan beruntun yang mengolah data piksel menjadi keputusan navigasi. Dapat diamati bahwa setiap tahap pemrosesan bekerja secara berurutan guna menjaga waktu respons sistem. Dengan demikian, diagram visual ini memvisualisasikan bagaimana umpan balik (\textit{feedback}) visual diterjemahkan menjadi aksi navigasi fisik. Rincian tahapan pada alur visual tersebut diuraikan sebagai berikut:
	
	\begin{itemize}
		\item Webcam USB menangkap aliran data optik mentah ke GStreamer.
		\item GStreamer mereduksi ukuran piksel citra sesuai kapabilitas komputasi CPU Raspberry Pi 5 dan menampungnya ke dalam \textit{Frame Queue}.
		\item \textit{YOLO Consumer} mengambil citra tersebut dari antrean, mengeksekusi komputasi \textit{deep learning}, lalu mengirimkan sekumpulan kotak pembatas objek (\textit{bounding box}) menuju \textit{Detection Queue}.
		\item \textit{Autonomy Controller} memproses data hasil observasi objek tersebut melalui matriks kondisi terkini pada \textit{state machine}, lalu menghasilkan vektor navigasi 3 sumbu berupa kecepatan dan yaw (\textit{VelocityNedYaw}).
	...
	\begin{figure}[H]
		...
		\caption{Blok Diagram Telemetri ke React}
		\label{fig:bloktelemtoreact}
	\end{figure}
	
	Gambar \ref{fig:bloktelemtoreact} menunjukkan alur pemrosesan data telemetri dari flight controller Pixhawk hingga dirender pada antarmuka operator. Berdasarkan implementasi tersebut, data dikirim secara asinkron menggunakan protokol \textit{WebSockets} untuk menghindari kelambatan render visual. Dapat diamati bahwa pemisahan alur telemetri ini menjaga kestabilan data telemetri yang ditampilkan ke pengguna. Dengan demikian, alur komunikasi ini dirancang agar operator dapat menerima metrik penerbangan aktual secara \textit{real-time}. Rincian tahapan pada alur telemetri tersebut diuraikan sebagai berikut:
	
	\begin{itemize}
		\item PX4 Flight Controller secara internal mengolah sinyal fisik sensor (IMU, Barometer, GPS) menjadi kumpulan paket status.
		\item \textit{MAVSDK Listener} pada Raspberry Pi terus-menerus berlangganan paket dari drone lalu memperbarui \textit{State Dictionary}.
		\item Antarmuka Flask SocketIO mengambil jepretan (\textit{snapshot}) matriks ini lima kali tiap detiknya.
...
Implementasi \textit{hardware} berfokus pada integrasi komponen komputasi dan sensor tambahan ke dalam drone yang telah terakit. Fokus utama pada tahap ini adalah memastikan distribusi daya dan jalur komunikasi antar perangkat tambahan berjalan stabil tanpa mengganggu stabilitas drone.

Realisasi fisik dari purwarupa drone otonom ini dilakukan dengan mewujudkan hasil desain mekanik, sistem distribusi daya, konfigurasi komunikasi data, serta konfigurasi sensor ke dalam wujud fisik drone yang aktual. Integrasi perangkat keras ini bertujuan untuk memvalidasi bahwa seluruh rancangan teoritis yang telah disusun dapat berfungsi secara harmonis pada kondisi lingkungan nyata. Oleh karena itu, tahap perwujudan fisik ini menghubungkan seluruh rancangan konseptual CAD 3D dan skema kelistrikan menjadi sistem penunjang penerbangan yang terintegrasi secara utuh.

\begin{itemize}
	\item Konfigurasi drone dan Propulsi: Penelitian ini menggunakan drone Holybro S500 yang secara basis telah terakit secara \textit{custom} pada bagian rangka, motor \textit{brushless}, \textit{Electronic Speed Controller} (ESC), PDB, serta \textit{Flight Controller} Pixhawk 6C (sebagaimana ditunjukkan pada Gambar \ref{fig:drone_s500_real}). Implementasi fisik difokuskan pada penempatan dan pemasangan komponen otonom tambahan yang belum tersedia pada drone dasar.
	\begin{figure}[H]
		\centering
		\begin{subfigure}[b]{0.45\textwidth}
		...
		\end{subfigure}
		\caption{Realisasi fisik rangka drone Holybro S500 dari perspektif atas dan bawah}
		\label{fig:drone_s500_real}
	\end{figure}
	
	\item Integrasi Unit Komputasi dan Sensor: Raspberry Pi 5 dipasang sebagai \textit{companion computer} utama untuk menjalankan algoritma deteksi manusia. Unit ini dihubungkan dengan \textit{webcam} Logitech C922 melalui antarmuka USB 3.0 untuk mendukung transmisi data visual berkecepatan tinggi.
	\begin{figure}[H]
		\centering
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{Modifikasi sirkuit catu daya berupa penyolderan XT60 pada UBEC 5A dan pembuatan kabel Y-Splitter}
		\label{fig:ubec_dan_splitter}
	\end{figure}
...
\begin{itemize}
	\item \textbf{Alasan Penggunaan PD dibanding P murni:} Pengendalian yaw otonom rawan mengalami lonjakan berlebih (\textit{overshoot}) dan getaran rotasional kontinu akibat inersia drone saat berputar cepat di udara. Integrasi suku derivatif ($K_d$) memberikan efek pengereman (redaman) dinamis yang proporsional terhadap laju perubahan error, sehingga mempercepat waktu stabilisasi (\textit{settling time}) dan meniadakan osilasi yaw.
\end{itemize}
