...
\begin{enumerate}
	...
	\item Komunikasi: Remote Controller Skydroid T10, Kabel JST GH 1.25mm 6 Pin, USB type C dan Modem MiFi 4G.
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		Untuk mengatasi kendala tersebut, desain kelistrikan akhir diubah menggunakan modul UBEC (\textit{Universal Battery Elimination Circuit}) tipe \textit{step-down} kontinu 5A yang disuplai oleh baterai LiPo. Baterai LiPo memiliki tingkat pelepasan arus (\textit{C-Rating}) yang sangat tinggi sehingga memastikan Raspberry Pi 5 menerima tegangan 5V yang stabil tanpa \textit{drop}. Pada implementasinya, konfigurasi desain kelistrikan akhir berbasis UBEC ini mendukung dua mode operasional:
...
\begin{enumerate}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Dudukan Utama Kompartemen 1}
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Dudukan Utama Kompartemen 2}
	...
	\begin{figure}[H]
		...
		\caption{Desain 2D dan Dimensi Dudukan Utama Kompartemen}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Plat Pengikat Kompartemen 1}
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Plat Pengikat Kompartemen 2}
	...
	\begin{figure}[H]
		...
		\caption{Desain 2D dan Dimensi Plat Pengikat Kompartemen}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Retainer Baterai LiPo 1}
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Retainer Baterai LiPo 2}
	...
	\begin{figure}[H]
		...
		\caption{Desain 2D dan Dimensi Retainer Baterai LiPo}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Casing Pelindung Companion Computer 1}
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Casing Pelindung Companion Computer 2}
	...
	\begin{figure}[H]
		...
		\caption{Desain 2D dan Dimensi Casing Pelindung Companion Computer}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Pelindung Modul Regulator Daya 1}
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Pelindung Modul Regulator Daya 2}
	...
	\begin{figure}[H]
		...
		\caption{Desain 2D dan Dimensi Pelindung Modul Regulator Daya}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Dudukan Belakang Kamera 1}
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Dudukan Belakang Kamera 2}
	...
	\begin{figure}[H]
		...
		\caption{Desain 2D dan Dimensi Dudukan Belakang Kamera}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Penjepit Depan Kamera 1}
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Desain 3D Penjepit Depan Kamera 2}
	...
	\begin{figure}[H]
		...
		\caption{Desain 2D dan Dimensi Penjepit Depan Kamera}
...
\begin{itemize}
	\item Branch Streaming: Menggunakan perangkat lunak \textit{encoder} H.264 (atau \textit{hardware encoder} \texttt{v4l2h264enc}) untuk mentransmisikan video latensi rendah via protokol UDP pada \textit{port} 5600, yang ditujukan untuk \textit{Ground Control Station} (GCS) eksternal. Cabang ini juga menyediakan \textit{fallback} berupa aliran \textit{Motion JPEG} (MJPEG) pada \textit{endpoint} \textit{REST API} untuk pemantauan antarmuka berbasis web. Perlu dicatat bahwa sistem \textit{backend} tidak melakukan perekaman video di atas wahana (\textit{onboard video recording}) guna mereduksi beban komputasi CPU, aktivitas \textit{storage I/O}, serta memperpanjang usia pakai kartu memori (\textit{SD-card}) pada Raspberry Pi 5. Direktori \texttt{/recordings} disediakan hanya untuk akses pembacaan berkas (\textit{file access only}) dan tidak ditulis secara aktif oleh sistem.
...
Modul deteksi berfungsi menerjemahkan data piksel mentah menjadi koordinat target spasial. Berdasarkan hasil analisis dan tahapan optimasi, sistem ini menggunakan model YOLOv8n standar yang telah melalui proses \textit{fine-tuning} khusus untuk deteksi kelas tunggal manusia (\textit{person-only}) menggunakan dataset COCO yang difilter dengan total boks anotasi sebanyak 244.799 instans target. Model final PyTorch (\texttt{best.pt}) berukuran 6,0 MB tersebut kemudian diekspor ke dalam format ONNX dengan resolusi input sebesar 320x320 piksel (\texttt{320n.onnx}) untuk diimplementasikan pada perangkat \textit{edge} Raspberry Pi 5.

Pemilihan resolusi input 320x320 piksel dengan format ONNX didasarkan pada titik temu (\textit{trade-off}) paling optimal antara performa komputasi dan akurasi deteksi. Strategi pemangkasan ruang prediksi menjadi kelas tunggal ini mampu mendongkrak nilai mAP@50 secara signifikan hingga mencapai 0,757 (meningkat sekitar 13\% dibandingkan model \textit{baseline} multi-kelas COCO) serta meraih tingkat \textit{Precision} sebesar 80,9\%. Reduksi ukuran input dan pemfokusan kelas ini sangat krusial untuk meminimalkan latensi pemrosesan (\textit{inference time}) pada lingkungan perangkat \textit{single-board computer}, sehingga sirkuit asinkron \textit{YOLO Consumer} tetap mampu memasok data secara konstan demi menjaga stabilitas responsivitas dari \textit{loop} kontrol otonom drone.

Proses deteksi dimulai saat \textit{YOLO Consumer} mengambil citra dari \textit{frame queue} dan memprosesnya menggunakan model tersebut melalui \textit{ONNX Runtime}. Untuk menjaga stabilitas penentuan target di lingkungan dinamis, sistem mengimplementasikan algoritma pemeringkatan (\textit{scoring}) dan penguncian target (\textit{target lock}) \cite{BarShalom2001Tracking}. Kandidat target dievaluasi berdasarkan kombinasi pembobotan: luas area \textit{bounding box} (45\%), tingkat keyakinan atau \textit{confidence} (25\%), kedekatan dengan titik tengah \textit{frame} (20\%), dan konsistensi pelacakan spasial (10\%). Sistem akan menahan fokus (\textit{lock}) pada target terpilih dan tidak akan berpindah ke objek manusia lain kecuali objek baru memiliki skor minimal 35\% lebih tinggi. Hal ini diterapkan guna mencegah fenomena transisi target yang tidak stabil (\textit{target ping-pong}). Hasil akhir deteksi secara asinkron didistribusikan ke dalam \textit{detection queue}.
...
\end{figure}
	...
	Modul deteksi dirancang untuk menerjemahkan data piksel mentah dari sensor visual menjadi koordinat target spasial yang akan digunakan oleh sistem kendali terbang. Arsitektur sistem otonom ini mengonfigurasikan model YOLOv8n (\textit{Nano}) yang disederhanakan menjadi detektor kelas tunggal khusus manusia (\textit{person-only}). Model dikonversi ke dalam format ONNX (\textit{Open Neural Network Exchange}) dengan perancangan dimensi resolusi input sebesar 320x320 piksel.
	
	Perancangan resolusi input 320x320 piksel dan pemilihan mesin inferensi \textit{ONNX Runtime} dispesifikasikan sebagai batasan desain (\textit{design constraint}) untuk menjamin efisiensi alokasi daya komputasi pada \textit{companion computer} Raspberry Pi 5. Dimensi ini dirancang agar pemrosesan visi komputer tidak menimbulkan latensi yang dapat menghambat pasokan data koordinat ke sirkuit kendali. Pemotongan struktur arsitektur jaringan menjadi kelas tunggal bertujuan mengeliminasi beban komputasi ekstra untuk klasifikasi objek non-target, sehingga seluruh kapasitas ekstraksi fitur fokus pada kontur dan siluet manusia. Hal ini dirancang untuk menjaga stabilitas pasokan data koordinat agar selaras dengan frekuensi \textit{loop} kontrol otonom utama yang bekerja pada 20 Hz.
	
	Alur logika pemrosesan dirancang mengalir secara asinkron, di mana komponen \\
	\texttt{YOLOConsumer} mengekstraksi citra dari \textit{frame queue} untuk diumpankan ke mesin inferensi. Guna menangani ketidakpastian kondisi di lapangan dan menjaga kestabilan navigasi, sistem mengintegrasikan rancangan algoritma pemeringkatan (\textit{scoring}) dan penguncian target (\textit{target lock}) sebelum perintah kecepatan diterbitkan. Matriks evaluasi kandidat target dirancang menggunakan kombinasi pembobotan linear sebagai berikut:
	
	\begin{itemize}
		\item Luas area boks pembatas (\textit{bounding box}) sebesar 45\%, sebagai indikator kedekatan jarak fisik target terhadap sensor kamera.
	...
	Sistem dirancang untuk mengunci (\textit{lock}) satu target utama dan mengabaikan kehadiran orang lain di sekitarnya. Fokus pelacakan hanya akan berpindah jika ada target manusia baru yang skor evaluasinya minimal 35\% lebih tinggi dari target saat ini. Aturan penahan ini sengaja diterapkan agar drone tidak kebingungan dan bolak-balik berganti sasaran dalam waktu singkat, yang dapat membuat arah terbangnya menjadi tidak stabil. Setelah target dipastikan, titik koordinat lokasinya akan dikirimkan ke dalam antrean sistem (\textit{detection queue}) agar drone dapat langsung bergerak menghampiri target secara otomatis.
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item \texttt{SCOUT\_DOCUMENT}: Drone menahan posisinya di udara (\textit{hold setpoint}) di dekat korban, memvalidasi radius eksklusi koordinat geospasial, dan mengambil dokumentasi (\textit{screenshot}) visual-spasial.
...
\begin{itemize}
	...
	\item Battery Failsafe: Telemetri baterai dipantau terus-menerus. Jika parameter menyentuh ambang batas kritis (< 15\%), sistem mengabaikan semua tugas dan memerintahkan tindakan mode \textit{Return to Launch} (RTL) atau \texttt{SCOUT\_RETURN\_HOME}.
...
\begin{itemize}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\linewidth]{bab3/integrasi_webcam_rpi5_depan} 
	...
	\begin{enumerate}
		...
		\begin{figure}[H]
			...
			\begin{subfigure}[b]{0.45\textwidth}
				\centering
				\includegraphics[width=0.9\linewidth]{bab3/konfig_baterai_tunggal_nc} % Sesuaikan path
				\caption{Realisasi Fisik Konfigurasi Baterai Tunggal (Paralel) Tanpa Penutup}
				\label{fig:baterai-tunggal-real-no-case}
			\end{subfigure}
			\hfill
			\begin{subfigure}[b]{0.45\textwidth}
				\centering
				\includegraphics[width=0.9\linewidth]{bab3/konfig_baterai_tunggal_wc} % Sesuaikan path
				\caption{Realisasi Fisik Konfigurasi Baterai Tunggal (Paralel) Dengan Penutup}
		...
		\end{figure}
	\end{enumerate}
	Melalui implementasi modul UBEC 5A pada kedua konfigurasi tersebut, suplai tegangan 5V ke Raspberry Pi terbukti tetap stabil selama proses inferensi berlangsung, yang mana hal ini berhasil mengatasi masalah malfungsi daya pada purwarupa awal.
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Jalur serial USB Tampak Depan}
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Jalur serial USB Tampak Samping}
			\label{fig:serial-px4-rpi5-samping}
		\end{subfigure}
		\vspace{0.5cm} 
		\begin{subfigure}[b]{0.45\textwidth}
			\centering
			\includegraphics[width=0.7\textwidth]{bab3/serial_uart_px4_rpi5_depan} 
			\caption{Jalur serial (UART) Tampak Depan}
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Jalur serial (UART) Tampak Samping}
			\label{fig:serial-uart-px4-rpi5-samping}
		\end{subfigure}
		\caption{Perbandingan jalur komunikasi serial USB dan UART antara Pixhawk dan Raspberry Pi 5}
		\label{fig:komunikasi-px4-rpi5-lengkap}
	\end{figure}
	...
	\item Instalasi Modem MiFi 4G: Untuk mendukung komunikasi \textit{dual-mode}, sebuah modem MiFi 4G (\textit{wireless}) ditempatkan pada ruang kosong di dalam komponen 3D pelindung modul suplai daya Raspberry Pi. Penempatan ini mempermudah tata letak dan meminimalisir risiko interferensi kabel pada sensor GPS
	\begin{figure}[H]
		\centering
		\begin{subfigure}[b]{0.45\textwidth}
			\centering
			\includegraphics[width=0.9\linewidth]{bab3/modem_mifi_4g}
			\caption{Modem MiFi 4G yang digunakan}
		...
		\begin{subfigure}[b]{0.45\textwidth}
			...
			\caption{Penempatan Modem MiFi 4G di dalam kompartemen pelindung daya}
...
\begin{lstlisting}[language=bash, caption={Struktur direktori fungsional perangkat lunak sistem drone otonom}]
	...
	|   `-- /recordings       # Akses berkas rekaman (tidak ditulis aktif oleh backend, video H264 langsung via UDP guna meminimalkan beban CPU, I/O, dan SD-card)
...
Skenario misi pencarian (\textit{Scout Mission}) mengatur tahapan gerak drone menggunakan diagram keadaan (\textit{State Machine}). Mengacu pada Listing \ref{lst:scoutstate}, sistem pertama-tama bertugas sebagai penjaga keamanan (\textit{failsafe}). Jika sisa baterai berada di bawah batas aman (\texttt{BATTERY\_RTL\_THRESHOLD}), otonom backend akan langsung memicu perintah \textit{Return to Launch} (RTL) bawaan PX4 secara mandiri untuk menghentikan misi dan memulangkan drone. Jika kondisi baterai aman, program akan menghitung dan membatasi kecepatan naik-turun drone (\texttt{vz}). Setelah itu, drone akan beroperasi sesuai dengan tahapan aktif saat itu, mulai dari proses menyapu mencari korban (\texttt{SCAN}), menghampiri korban (\texttt{APPROACH}), mengambil dokumentasi foto (\texttt{DOCUMENT}), hingga akhirnya melakukan pergeseran posisi secara lateral (\texttt{DISPLACING}) sebelum kembali memindai (\texttt{SCAN}).

\begin{lstlisting}[language=Python, caption={Perpindahan diagram keadaan (\textit{State Machine}) skenario pencarian udara}, label={lst:scoutstate}]
	if self.state_dict.get("battery_pct", 100) < self.BATTERY_RTL_THRESHOLD:
...
\begin{enumerate}
	...
	Integritas misi dilindungi oleh sistem pengawasan (\textit{watchdog}). Apabila terjadi kondisi kritis seperti kapasitas baterai di bawah ambang batas atau kehilangan koneksi, sistem secara otomatis akan memicu prosedur \textit{Return to Launch} (RTL) atau \textit{Return Home}. Selain itu, operator memegang otoritas tertinggi melalui \textit{Remote Controller} untuk melakukan \textit{override} manual kapan saja dalam kondisi darurat.
	
	\item Pengawasan Operator (\textit{Monitoring}) \\
	Seluruh data operasional, status terbang, dan tangkapan visual dipancarkan secara kontinu ke \textit{frontend monitoring} melalui koneksi jaringan nirkabel (\textit{Wi-Fi} lokal atau modem \textit{MiFi 4G}), memungkinkan operator melakukan pengawasan misi pencarian secara presisi.
\end{enumerate}
